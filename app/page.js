...
import { ArrowUpRight, BadgeCheck, Bug, CheckCircle2, Database, Download, ExternalLink, Globe2, Mail, MapPin, Phone, ShieldCheck, TestTube2, Zap } from 'lucide-react';
...
const featuredProjects = [
  {
    title: 'DAR/DALC — Tele Prospecting Module',
    label: 'Tele Marketer',
    year: '2026',
    type: 'Enterprise Dashboard QA',
    description: 'Tested tele prospecting workflows inside the DAR/DALC enterprise platform, including lead handling, sales rep assignment, appointment booking, and call outcome actions.',
    highlights: ['Multi-role workflow', 'Dashboard QA', 'Lead flow testing', 'Appointment actions'],
    image: '/projects/tele-marketer.png',
    imageClass: 'object-cover object-top',
    source: 'CV project visual',
  },
  {
...
const moreProjects = [
  ...
  {
    title: 'Rosper — Social Growth App',
    year: '2025',
    type: 'Mobile QA',
    description: 'Validated real-time chat, streak challenges, swipe-to-match, and community features across iOS and Android before public release.',
    highlights: ['10K+ downloads', 'Firebase Crashlytics', 'Android Studio', 'Xcode Simulator'],
  },
];
    ...
    <main id="home" className="min-h-screen overflow-hidden">
      ...
      <section id="about" className="section-pad py-20">
        <div className="container">
          <SectionHeading eyebrow="About" title="Quality-focused engineer for web, ERP, AI, and mobile products" description="My work combines structured test planning, defect lifecycle management, automation, API testing, and database validation to protect product stability before release." />
          <div className="grid gap-6 lg:grid-cols-3">
            {[{ icon: TestTube2, title: 'Test Strategy', text: 'Test planning, test case design, risk-based coverage, regression suites, and release verification.' }, { icon: Zap, title: 'Automation', text: 'Playwright and Selenium automation for critical workflows and faster retest cycles.' }, { icon: Database, title: 'API & Data QA', text: 'REST API validation with Postman and Swagger, plus SQL and Prisma Studio data verification.' }].map((item) => (
              <Card key={item.title}>
      ...
      <section id="experience" className="section-pad py-20">
        <div className="container">
          <SectionHeading eyebrow="Experience" title="Professional QA experience" description="Hands-on testing experience across enterprise platforms, ERP systems, web applications, mobile applications, payment flows, and backend APIs." />
              ...
              <Card key={job.company} className="grid gap-6 md:grid-cols-[0.32fr_1fr_0.18fr]">
                ...
                <ul className="grid gap-3">
                  {job.points.map((point) => (
                    <li key={point} className="flex gap-3 text-slate-300"><CheckCircle2 className="mt-1 shrink-0 text-cyan-200" size={17} /> {point}</li>
                  ))}
                </ul>
      ...
      <section id="projects" className="section-pad py-20">
        <div className="container">
          <SectionHeading eyebrow="Projects" title="Featured QA projects with screenshots" description="A polished project showcase using the screenshots for Tele Marketer, Dubai Business Enterprise, Establisher, and Cell it, plus CV-based projects." />
              ...
              <Card key={project.title} className="flex flex-col overflow-hidden p-3">
                <div className={`relative aspect-[16/10] overflow-hidden rounded-[24px] border border-white/10 ${project.imageWrapClass || 'bg-white/[0.03]'}`}>
                  ...
                  <div className="absolute left-4 top-4 flex flex-wrap gap-2">
                    <span className="rounded-full border border-white/10 bg-black/55 px-3 py-1 text-[11px] font-semibold tracking-wide text-white backdrop-blur-xl">{project.label}</span>
                    <span className="rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1 text-[11px] font-semibold text-cyan-100 backdrop-blur-xl">{project.source}</span>
                  </div>
                </div>
                ...
                <div className="px-3 pb-3 pt-5">
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    {project.highlights.map((tag) => <span key={tag} className="rounded-full bg-white/5 px-3 py-1 text-xs text-slate-300">{tag}</span>)}
                  </div>
                  {project.link ? <a href={project.link} target="_blank" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-cyan-200" rel="noreferrer">Visit Website <ExternalLink size={15} /></a> : null}
                </div>
              </Card>
            ))}
          </div>
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            {moreProjects.map((project) => (
              <Card key={project.title} className="flex min-h-[290px] flex-col">
                <div className="flex items-center justify-between gap-4">
                  <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-white/85">{project.type}</span>
                  <span className="text-sm text-slate-500">{project.year}</span>
                </div>
      ...
      <section id="skills" className="section-pad py-20">
        <div className="container">
          <SectionHeading eyebrow="Skills" title="Technical QA toolkit" description="A practical stack for test design, automation, API validation, defect tracking, database checks, and release assurance." />
          <Card>
            <div className="flex flex-wrap gap-3">
              {skills.map((skill) => <span key={skill} className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-slate-300 transition hover:border-cyan-200/30 hover:bg-cyan-200/10 hover:text-white">{skill}</span>)}
            </div>
          </Card>
        </div>
      </section>
        ...
        <div className="container grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <Card>
            ...
            <ul className="mt-6 grid gap-4">
              {achievements.map((item) => <li key={item} className="flex gap-3 text-slate-300"><CheckCircle2 className="mt-1 shrink-0 text-cyan-200" size={17} /> {item}</li>)}
            </ul>
          </Card>
        </div>
      </section>
      ...
      <section id="contact" className="section-pad py-20">
        <div className="container grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            <SectionHeading align="left" eyebrow="Contact" title="Let’s discuss your QA needs" description="Use the contact form for QA roles, manual testing, automation, API testing, database validation, or release verification work." />
      ...
      <footer className="border-t border-white/10 section-pad py-8">
        <div className="container flex flex-col justify-between gap-3 text-sm text-slate-500 md:flex-row">
          <p>© 2026 Hamad Ur Rehman. All rights reserved.</p>
          <p className="inline-flex items-center gap-2"><Globe2 size={16} /> Built with Next.js and Tailwind CSS.</p>
        </div>
      </footer>
    </main>
  );
